...
import Tab1 from "@/views/site/modules/moduleThree/tab-content/tab-1.vue";
import Tab3 from "@/views/site/modules/moduleThree/tab-content/tab-3.vue";
...
export default {
  ...
  data() {
    return {
      ...
      sectionContentTree: [
        {
          id: 0,
          name: "Общие способы защиты от ОРИ",
          contentList: [
            {
              id: 0,
              name: "Общие способы защиты от ОРИ test",
            },
            {
              id: 1,
              name: "Меры гигиены для защиты от ОРИ и COVID-19",
            },
          ],
        },
        {
          id: 1,
          name: "Респираторный этикет",
          contentList: [
            {
              id: 2,
              name: "Респираторный этикет",
  ...
  methods: {
    detailedPages() {
      this.$router.push({ path: "/module-three" });
    },
    ...
    async selectContent(sectionContentId, contentId) {
      ...
      localStorage.setItem("activeTabThree", Number(this.activeTab));
      try {
        let data = {
          moduleId: 3,
    ...
    activeTabList(tab) {
      localStorage.setItem("activeTabThree", Number(tab));
      this.activeTab = Number.parseInt(localStorage.getItem("activeTabThree"));
  ...
  created() {
    this.activeTab = Number.parseInt(localStorage.getItem("activeTabThree"))
      ? Number.parseInt(localStorage.getItem("activeTabThree"))
      : 0;
  },
};
</script>
